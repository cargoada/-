    ...
    with st.expander("🗑️ 清理歷史資料"):
        ...
        if st.button("⚠️ 確認刪除", type="primary"):
            ...
            if not df_sess.empty:
                ...
                keep_sess = (df_sess['temp_dt'] >= cutoff) | (pd.to_numeric(df_sess.get('invoice_id', 0), errors='coerce').fillna(0) == 0)
            ...
            if not df_inv.empty:
                df_inv['temp_dt'] = pd.to_datetime(df_inv.get('created_at', datetime.now().isoformat()), errors='coerce')
                keep_inv = (df_inv['temp_dt'] >= cutoff) | (pd.to_numeric(df_inv.get('is_paid', 0), errors='coerce').fillna(0) == 0)
...
with tab1:
    ...
    if not df_sess.empty:
        ...
        with st.expander("🔍 查看「待結算」詳細清單"):
            if pending_income > 0:
                chart_pending_mask = (chart_df['end_dt'] < current_time) & (chart_df['safe_invoice_id'] == 0)
                pending_display = chart_df[chart_pending_mask].copy()
                
                show_list = [{"日期": r['start_dt'].strftime('%m/%d'), "學生": r.get('name', '未知'), "金額": int(r.get('amount', 0))} for _, r in pending_display.iterrows()]
...
with tab3:
    ...
    if st.button("⚡ 一鍵結算 (自動分月開單)", type="primary"):
        df_sess['end_dt'] = pd.to_datetime(df_sess['start_time'], errors='coerce') # 用 start_time 較安全
        ...
        if not pending_df.empty:
            ...
            for (sid, m_str), group in groups:
                total_amt = sum(((pd.to_datetime(r['end_time']) - pd.to_datetime(r['start_time'])).total_seconds() / 3600) * int(r.get('actual_rate', 0)) for _, r in group.iterrows())
